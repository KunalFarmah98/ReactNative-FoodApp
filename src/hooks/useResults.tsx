...
const useResults = () => {
    ...
    const getFavourites = async () => {
        await db.transaction(txn => {
            txn.executeSql('CREATE TABLE IF NOT EXISTS fav(id VARCHAR(255) PRIMARY KEY, name VARCHAR(255), image VARCHAR(65535), rating FLOAT(0), stars FLOAT(0), price VARCHAR(255))', []);
        })
        await db.transaction((txn) => {
            txn.executeSql("SELECT * FROM fav", [], function (tx, res) {
                ...
                for (let i = 0; i < res.rows.length; ++i) {
                    ...
                    favs.push(new Restaurant(fav.id, fav.name, fav.stars, fav.rating, fav.price, fav.image, true));
                }
            });
        });
